...
//    System.out.println("catList.jsp 처음 호출시 colname : "+colname);
//    System.out.println("catList.jsp 처음 호출시 findtext : "+findtext);
    // option 태그를 선택하고 검색어를 입력하지 않으면 findtext는 null이 아닌 ""으로 넘어온다
    // option 태그를 선택하지 않았을때 colname의 기본값은 seleted로 설정해놓음

    // 처음으로 catList.jsp를 호출하거나, 옵션태그를 선택하지 않거나, 검색단어를 입력하지 않았을때
    // if문의 조건을 하나라도 충족하면 검색을 실행할 필요가 없으므로 기본검색을 위한 설정값을 셋팃
    if (colname == null || colname.equals("selected") || findtext.equals("")){
        colname = "selected";
        findtext = "";
    }
//    System.out.println("catList.jsp에서 옵션태그 선택 후 colname : "+colname);
//    System.out.println("catList.jsp에서 검색창에 입력한 검색어 findtext : "+findtext);
...
//    ArrayList<CatBoard> cblists = catfdao.catList(searchList);

    // 페이징 부분

    int bdcnt = catfdao.countBoard();
    ...
    ArrayList<CatBoard> cblists = catfdao.catList(searchList, startnum, endnum);
    ...
    <div class="main">
        ...
        <div class="row" style="margin: 10px 0 20px 0">
            ...
            <div class="col-sm-9 row">
                <%for (int i = 0; i < cblists.size(); i++) { %>
                <div style="width: 150px; height: 150px; margin: 20px;">
                    <div class="thumbnail bootsnipp-thumb" style="padding:0;">
                        <div class="imgwrap" style="height: 100px;">
                            <a href="catView.jsp?bdno=<%=cblists.get(i).getBdno()%>">
                                <img class="danlazy" src="showimg.jsp?f=<%=cblists.get(i).getFile1()%>" width="100%" height="100%">
                            </a>
                        </div>
                        <%--제목,날짜,조회수 표시할 부분--%>
                        <div style="width: 150px;">
                            <div>
                                <span class="text-left" style="overflow: hidden ; text-overflow: ellipsis; white-space: nowrap;  width: 150px; height: auto; display: inline-block;  ">
                                    <%--게시글 상세보기로 넘어가는 링크작성--%>
                                    <a href="catView.jsp?bdno=<%=cblists.get(i).getBdno()%>"><%=cblists.get(i).getTitle()%></a>
                                </span>
                            </div>
                            <div class="text-leftx" style="font-size: 10pt">
                                <span>조회수 <%=cblists.get(i).getViews()%> |</span>
                                <span>추천수 <%=cblists.get(i).getLikes()%></span>
                                <span><%=cblists.get(i).getRegdate().substring(0,10)%></span>
                            </div>
                        </div>
                    </div>
                    <%--                    <%if (i >= 5) { %>--%>
        ...
        <%--검색창--%>
        <form name="catList" method="post" action="catList.jsp?cpage=1">
            <div align="center">
                <table>
                    <tr>
